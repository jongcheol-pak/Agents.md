...
- **문서**: 새 세션 시작 시 `plan.md`·`AGENTS.md` 확인, 기능 변경 시 `README.md` 갱신. **완료된 작업의 영구 기록은 git 커밋이 유일하다** — `plan.md`는 다음 계획 때 교체되므로 기록처가 아니다.
...
- 검증 실패 시 수정→재검증을 반복하되, **동일 이슈로 3회 실패하면 멈추고 상황을 보고**한다 (이 중단 조건이 아래 "계속 진행" 원칙보다 우선한다). 단 **발동된 스킬·승인된 plan이 자체 재시도 한계를 정의하면 그 카운터를 따른다.**
