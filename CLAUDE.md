...
- git 저장소가 아닌 폴더에서의 파일 덮어쓰기·삭제·이름 변경 (git이 없으면 모든 변경이 되돌릴 수 없다 — 사전 백업 또는 사용자 확인 후 진행)
  - 사전 백업은 시스템 임시 폴더에 둔다(프로젝트 폴더를 더럽히지 않기 위함). 세션이 끝나도 남지만 **디스크 공간이 부족하면 OS가 정리할 수 있으므로**, 오래 보존할 백업이 필요하면 사용자에게 위치를 확인한다. 단 발동된 스킬이 자체 백업 규약(위치·보존·정리)을 정의하면 그것을 따른다.
